...
BPLUTBlockNoDivKernel::BPLUTBlockNoDivKernel(CUmodule aModule, dim3 aGridDim, dim3 aBlockDim)
        : CudaKernel("backProjectionLUTVoxelBlockNoDiv", aModule, aGridDim, aBlockDim, 0)
{

}

BPLUTBlockNoDivKernel::BPLUTBlockNoDivKernel(CUmodule aModule)
        : CudaKernel("backProjectionLUTVoxelBlockNoDiv", aModule, make_dim3(1, 1, 1), make_dim3(4, 4, 4), 0)
{

}

void BPLUTBlockNoDivKernel::MatrixVector3Mul(float4x4 M, float3& v, float2& erg)
{
    erg.x = M.m[0].x * v.x + M.m[0].y * v.y + M.m[0].z * v.z + 1.f * M.m[0].w;
    erg.y = M.m[1].x * v.x + M.m[1].y * v.y + M.m[1].z * v.z + 1.f * M.m[1].w;
}

int2 BPLUTBlockNoDivKernel::computeSupport(Volume<float> &vol,
                                           Projection &proj,
                                           int index,
                                           int3 voxelBlockDim,
                                           int supportsize,
...
float BPLUTBlockNoDivKernel::operator()(int proj_x,
                                   int proj_y,
                                   float lambda,
                                   float maxOverSample,
