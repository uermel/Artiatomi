...
FPDistSlicedKernel::FPDistSlicedKernel(CUmodule aModule, dim3 aGridDim, dim3 aBlockDim)
        : CudaKernel("distanceImageLUTNoDiv", aModule, aGridDim, aBlockDim, 0)
{

}

FPDistSlicedKernel::FPDistSlicedKernel(CUmodule aModule)
        : CudaKernel("distanceImageLUTNoDiv", aModule, make_dim3(1, 1, 1), make_dim3(4, 4, 4), 0)
{

}

void FPDistSlicedKernel::MatrixVector3Mul(float4x4 M, float3& v, float2& erg)
{
    erg.x = M.m[0].x * v.x + M.m[0].y * v.y + M.m[0].z * v.z + 1.f * M.m[0].w;
    erg.y = M.m[1].x * v.x + M.m[1].y * v.y + M.m[1].z * v.z + 1.f * M.m[1].w;
}

int2 FPDistSlicedKernel::computeSupport(Volume<float> &vol,
                                       Projection &proj,
                                       int index,
                                       int3 voxelBlockDim,
                                       int supportsize,
...
float FPDistSlicedKernel::operator()(int proj_x,
                                    int proj_y,
                                    float lambda,
                                    float maxOverSample,
                                     CudaPitchedDeviceVariable& distanceMap,
                                    ...
                                    Volume<float>* vol)
{
    ...
    size_t stride = distanceMap.GetPitch();
