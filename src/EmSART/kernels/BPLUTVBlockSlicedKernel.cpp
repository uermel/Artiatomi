...
BPLUTVBlockSlicedKernel::BPLUTVBlockSlicedKernel(CUmodule aModule, dim3 aGridDim, dim3 aBlockDim)
        : CudaKernel("backProjectionLUTVoxelBlockNoDivSlice", aModule, aGridDim, aBlockDim, 0)
{

}

BPLUTVBlockSlicedKernel::BPLUTVBlockSlicedKernel(CUmodule aModule)
        : CudaKernel("backProjectionLUTVoxelBlockNoDivSlice", aModule, make_dim3(1, 1, 1), make_dim3(4, 4, 4), 0)
{

}

void BPLUTVBlockSlicedKernel::MatrixVector3Mul(float4x4 M, float3& v, float2& erg)
{
...
int2 BPLUTVBlockSlicedKernel::computeSupport(Volume<float> &vol,
                                           Projection &proj,
                                           int index,
                                           int3 voxelBlockDim,
                                           int supportsize,
...
float BPLUTVBlockSlicedKernel::operator()(int proj_x,
                                        int proj_y,
                                        float lambda,
                                        float maxOverSample,
                                        ...
                                        Volume<float>* vol)
{
    ...
    int gridX = (int)ceilf(vol->GetDimension().x/4);
    int gridY = (int)ceilf(vol->GetDimension().y/4);
    int gridZ = (int)ceilf(vol->GetDimension().z/4);
                       ...
                       float sliceThickness)
{
    ...
    //printf("==============================================================\n");
    //Set constant values
    int2 supportTest = kernel.computeSupport(vol, proj, index, make_int3(2, 2, 2), support, (int)maxOverSample);
