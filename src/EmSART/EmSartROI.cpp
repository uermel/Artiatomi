...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        DeviceVolume deviceVolume(volDimU);

//        CudaArray3D vol_array_card(arrayFormat, (int)volSize.x, (int)volSize.y, (int)volSize.z, 1, 2);
//        CudaArray3D vol_array_dual(arrayFormat, (int)volSize.x, (int)volSize.y, (int)volSize.z, 1, 2);
//        CudaTextureObject3D texture_dual(CU_TR_ADDRESS_MODE_CLAMP, CU_TR_ADDRESS_MODE_CLAMP, CU_TR_ADDRESS_MODE_CLAMP, CU_TR_FILTER_MODE_LINEAR, 0, &vol_array_dual);
//        CudaSurfaceObject3D surface_card(&vol_array_card);
//        CudaSurfaceObject3D surface_dual(&vol_array_dual);
...
//        if (aConfig.FP16Volume)
//        {
//            vol_array_card.CopyFromHostToArray(volFP16->GetPtrToSubVolume(mpi_part));
...
//        else
//        {
        //deviceVolume.array_card().CopyFromHostToArray(vol->GetPtrToSubVolume(mpi_part));
...
/////////////////////////////////////
        ...
        // Init the particle volume
        int psize = 64;
        uint3 partDimU = make_uint3(psize, psize, psize);
        DeviceVolume deviceParticle(partDimU, CU_AD_FORMAT_FLOAT, true);
        DeviceVolume deviceMask(partDimU, CU_AD_FORMAT_FLOAT, true);
        DeviceVolume deviceMaskInv(partDimU, CU_AD_FORMAT_FLOAT, true);

        Volume<float> mask(partDimU);
        mask.LoadFromFile("/home/uermel/EMPIAR-10304/data/joint_reconstruction/initial_test/mask_fancy.em", 0);
        mask.Invert();
        deviceMask.HostToCard(&mask);

        Volume<float> maskinv(partDimU);
        maskinv.LoadFromFile("/home/uermel/EMPIAR-10304/data/joint_reconstruction/initial_test/mask_fancy_inv.em", 0);
        maskinv.Invert();
        deviceMaskInv.HostToCard(&maskinv);

//        CudaArray3D part_array_card(arrayFormat, (int)psize, (int)psize, (int)psize, 1, 2);
//        CudaArray3D part_array_dual(arrayFormat, (int)psize, (int)psize, (int)psize, 1, 2);
//        CudaTextureObject3D part_texture_dual(CU_TR_ADDRESS_MODE_CLAMP, CU_TR_ADDRESS_MODE_CLAMP, CU_TR_ADDRESS_MODE_CLAMP, CU_TR_FILTER_MODE_LINEAR, 0, &part_array_dual);
//        CudaSurfaceObject3D part_surface_card(&part_array_card);
//        CudaSurfaceObject3D part_surface_dual(&part_array_dual);



        // Get motl
        MotiveList motiveList("/home/uermel/EMPIAR-10304/data/joint_reconstruction/initial_test/initial_test_motl.em", 1, 1);
        motiveList.selectTomo(1);
        ...
        for (int partIdx = 0; partIdx < motiveList.GetParticleCount(); partIdx++) {
        ...
        auto dist_child = new float[proj.GetWidth()*proj.GetHeight()];
        ...
        for (int iter = 0; iter < 5; iter++) {
            for (uint projIdx = 0; projIdx < projCount; projIdx++) {
                ...
                bool useSNR = false;
                if (iter > 0) useSNR = true;

                reconstructor.ResetProjectionsDevice();

                reconstructor.ForwardProjectionFamily(vol,
                                                      particles,
                                                      deviceParticle,
                                                      deviceMask,
                                                      deviceVolume,
                                                      stackIdx, false, iter, true);

                reconstructor.DistanceChildren(vol, particles, deviceMask, stackIdx, false, iter, false);
                ...
                reconstructor.CompareSpecial(vol, projSource->GetProjection(stackIdx), stackIdx, true, iter);

                reconstructor.BackProjectionChildren(vol,
                                                   particles,
                                                   deviceParticle,
                                                   deviceMask,
                                                   stackIdx, 1, iter, useSNR);

                reconstructor.BackProjectionParent(vol,
                                                   deviceVolume,
                                                   stackIdx, 1, iter, useSNR);

                // Mean free in invmask
                deviceParticle.CardToVar();
                deviceMaskInv.CardToVar();

                deviceParticle.Mul(deviceMaskInv);
                float mean = deviceParticle.Sum() / deviceMaskInv.Sum();

                deviceParticle.CardToVar();
                deviceParticle.SubC(mean);
                deviceParticle.VarToCard();

                {
                    auto img = new float[particles[0]->GetSubVolumeSizeInVoxels(0)];
                    deviceParticle.array_card().CopyFromArrayToHost(img);
                    for (int i=0; i<particles[0]->GetSubVolumeSizeInVoxels(0); i++){
                        img[i] = img[i] * -1.f;
                    }
                    stringstream DP;
                    DP << "iter_particle_"  << stackIdx << "_" << iter << ".em";
                    emwrite(DP.str(), img, particles[0]->GetDimension().x,
                            particles[0]->GetDimension().y, particles[0]->GetDimension().z);
                    delete[] img;
                }
...
//                reconstructor.ResetProjectionsDevice();
//
//                reconstructor.ForwardProjectionFamilyNoCTF(vol,
//                                                           particles,
//                                                           deviceParticle,
//                                                           deviceVolume,
//                                                           stackIdx, false, iter, true);
...
//                }
//
//                reconstructor.BackProjectionParentNoCTF(vol,
//                                                        deviceVolume,
//                                                        stackIdx, 1, iter);
//
//                {
...
//                }
//
//                reconstructor.BackProjectionChildrenNoCTF(vol,
//                                                          particles,
//                                                          deviceParticle,
//                                                          stackIdx, 1, iter);
//
...
//            int stackIdx = indexList[projIdx];
//
//            reconstructor.CopyProjectionToDevice((float*)projSource->GetProjection(stackIdx));
//
//            reconstructor.BackProjectionChildrenNoCTF(vol,
//                                                  particles,
//                                                 deviceParticle,
...
//        }
//
//        Volume<float> voldummy(make_uint3(psize, psize, psize));
//        voldummy.LoadFromFile("/home/uermel/EMPIAR-10304/data/joint_reconstruction/initial_test/work/volume_part_meanfree.em", 0);
//
//        deviceParticle.array_card().CopyFromHostToArray(voldummy.GetPtrToSubVolume(0));
//
//        // FP
//        for (uint projIdx = 0; projIdx < projCount; projIdx++) {
//            int stackIdx = indexList[projIdx];
...
//            reconstructor.ResetProjectionsDevice();
//
//            reconstructor.ForwardProjectionChildrenNoCTF(vol,
//                                                  particles,
//                                                  deviceParticle,
//                                                  stackIdx, false, 0, true);
//
//            {
//                auto img = new float[proj.GetWidth()*proj.GetHeight()];
//                reconstructor.CopyProjectionToHost(img);
//                stringstream DP;
//                DP << "forward_" << stackIdx << "_" << 0 <<".em";
//                emwrite(DP.str(), img, proj.GetWidth(),
//                        proj.GetHeight());
//                delete[] img;
//            }
...
///////////////////////////////////////////////////////////////////////////////
        ...
        //vol->LoadFromFile("/home/Group/Share/lamella_test/test2/EmSART/testordner_ctf/holey_vol.em", 0);
        //vol_array_card.CopyFromHostToArray(vol->GetPtrToSubVolume(0));
...
//        {
//            // SNR based CTF-correction only after the first iteration with ad-hoc Wiener factor
//            bool useSNR = false;
//            if (iter > 0){
//                useSNR = true;
//            }
...
//                        if (aConfig.FP16Volume)
//                        {
//                            reconstructor.ForwardProjection(volFP16, texture_dual, index, volumeIsEmpty);
//                            if (mpi_part == 0)
//                            {
//                                reconstructor.Compare(volFP16, projSource->GetProjection(index), index, useSNR, iter);
//                            }
//                        }
//                        else
//                        {
//                            reconstructor.ForwardProjectionLUT(vol,
//                                                               surface_card,
//                                                               surface_dual,
//                                                               index, volumeIsEmpty, iter);
//
//
//
//                            if (mpi_part == 0)
...
//                                {
//                                    stringstream DP;
//                                    DP << "real_" << index << "_" << iter <<".em";
//                                    emwrite(DP.str(), (float*)projSource->GetProjection(index), proj.GetWidth(),
//                                            proj.GetHeight());
//                                }
//
//                                {
...
//                                }
////#endif
//                                reconstructor.Compare(vol, projSource->GetProjection(index), index, true, iter);
////#ifdef WRITEDEBUG
//
//                                {
//                                    auto img = new float[proj.GetWidth()*proj.GetHeight()];
...
//                    if (aConfig.FP16Volume)
//                    {
//                        reconstructor.BackProjection(volFP16, surface_card, index, (float)SIRTcount);
//                    }
//                    else
//                    {
//
...
//#endif
//
//                        reconstructor.BackProjectionLUT(vol,
//                                                        surface_card,
//                                                        surface_dual,
//                                                        texture_dual,
//                                                        index, (float)SIRTcount, iter, useSNR);
//#ifdef WRITEDEBUG
//                        {
//                            auto img = new float[vol->GetSubVolumeSizeInVoxels(0)];
//                            vol_array_card.CopyFromArrayToHost(img);
//                            stringstream DP;
//                            DP << "volume_" << index << "_" << iter << ".em";
//                            emwrite(DP.str(), img, vol->GetDimension().x,
//                                    vol->GetDimension().y, vol->GetDimension().z);
//                            delete[] img;
//                        }
...
//
//            if (aConfig.FP16Volume)
//                vol_array_card.CopyFromArrayToHost(volFP16->GetPtrToSubVolume(mpi_part));
//            else
//                vol_array_card.CopyFromArrayToHost(vol->GetPtrToSubVolume(mpi_part));
//
//            if (mpi_part == 0) printf("Done\n");
//        }
//
...
//                    {
//                        reconstructor.ConvertVolumeFP16(volTemp_h, surface_card, zSlice);
