...
using namespace Cuda;
    ...
    FPLUTKernel::FPLUTKernel(CUmodule aModule)
            : Cuda::CudaKernel("march", aModule, make_dim3(1,1,1), make_dim3(4, 4, 4), 0)
    {
...
BPLUTKernel::BPLUTKernel(CUmodule aModule, dim3 aGridDim, dim3 aBlockDim, bool fp16)
        : CudaKernel(fp16 ? "backProjectionFP16" : "backProjection", aModule, aGridDim, aBlockDim, 0)
{

}

BPLUTKernel::BPLUTKernel(CUmodule aModule, bool fp16)
        : CudaKernel(fp16 ? "backProjectionFP16" : "backProjection", aModule, make_dim3(1, 1, 1), make_dim3(4, 4, 4), 0)
{

}

float BPLUTKernel::operator()(int proj_x,
                              int proj_y,
                              float lambda,
                              float maxOverSample,
                              ...
                              float tmax)
{
    CUtexObject texObj = projection.GetTexObject();
    CUsurfObject surfObj = volume.GetSurfObject();
