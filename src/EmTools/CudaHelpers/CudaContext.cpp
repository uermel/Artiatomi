	...
	mDeviceID(deviceID), mCtxFlags(0), mHcuContext(0), mHcuDevice(0)
{
	...
	int active;
	cuDevicePrimaryCtxGetState(mHcuDevice, (uint*)&mCtxFlags, &active);
	//if (!active)
	{
		this->SetCurrent();
	}	
...
Cuda::CudaContext * Cuda::CudaContext::GetPrimaryContext(int aDeviceID)
{
	int deviceCount;
	// test whether the driver has already been initialized
	if (cuDeviceGetCount(&deviceCount) == CUDA_ERROR_NOT_INITIALIZED)
	...
	return new CudaContext(aDeviceID, true);
}
