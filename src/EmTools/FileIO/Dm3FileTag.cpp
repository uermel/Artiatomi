...
uint Dm3FileTag::GetSize(uint typeID)
{
	uint totalSize = 0;
	switch (typeID)
		{
...
uint Dm3FileTag::GetSizePixel(uint typeID)
{
	uint totalSize = 0;
	switch (typeID)
		{
...
//void Dm3FileTag::PrintValues(ostream& aStream)
//{
//	
//}

void Dm3FileTag::PrintSingleValue(ostream& aStream, uint aType, uint aStartPos)
{
	switch (aType)
	{
		case DTTI2:
...
int Dm3FileTag::GetSingleValueInt(uint aType, uint aStartPos)
{
	switch (aType)
	{
		case DTTI2:
...
float Dm3FileTag::GetSingleValueFloat(uint aType, uint aStartPos)
{
	switch (aType)
	{
		case DTTI2:
...
double Dm3FileTag::GetSingleValueDouble(uint aType, uint aStartPos)
{
	switch (aType)
	{
		case DTTI2:
...
string Dm3FileTag::GetSingleValueString(uint aType, uint aStartPos)
{
	switch (aType)
	{
	case DTTUI2:
...
ostream& operator<<(ostream &stream, Dm3FileTag& tag)
{
	uint bytesRead = 0;
	uint elemSize;
...
int Dm3FileTag::GetStructValueAsInt(uint aIndex)
{
