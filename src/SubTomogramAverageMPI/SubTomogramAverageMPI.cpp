...
int main(int argc, char* argv[])
{
	...
	for (int iter = aConfig.StartIteration; iter < aConfig.EndIteration; iter++)
	{
		...
		int size = ref.DimX;
		{
			...
			if (ref.DimX != particleSize || coverage_wedges.begin()->second->DimX != particleSize || mask.DimX != particleSize || ccmask.DimX != particleSize)
			{
				if (mpi_part == 0)
		...
		if (aConfig.MultiReference)
		{
			...
			for (int i = startParticle; i < endParticle; i++)
			{
				...
				for (int ref = 0; ref < refCount; ref++)
				{
					...
					{
						temp = aps[ref]->executeMaskFirst((float*)part.GetData(),
                                                         (float*)coverage_wedges[wedgeIdx]->GetData(),
                                                         (float*)overlap_wedges[wedgeIdx]->GetData(),
                                                         filterData,
                                                         mot.phi,
                                                         mot.psi,
                                                         mot.theta,
				...
				if (mpi_part == 0)
					cout << setprecision(0) << "I: " << (int)mot.partNr << " cc:  " << fixed << setprecision(3) << v.ccVal;
				if (mpi_part == 0)
					cout.unsetf(ios_base::floatfield);
				if (mpi_part == 0)
					cout << " phi: " << setw(6) << phi << " psi: " << setw(6) << psi << " the: " << setw(6) << theta << " shift: " << setw(2) << sx << "," << setw(2) << sy << "," << setw(2) << sz << endl;


				if (mpi_part == 0)
					cout << setprecision(0) << "I: " << (int)mot.partNr << " ref: " << fixed << setprecision(3) << v.ref + 1;
				if (mpi_part == 0)
					cout.unsetf(ios_base::floatfield);
				if (mpi_part == 0)
					cout << "     phi: " << setw(6) << v.rphi << " psi: " << setw(6) << v.rpsi << " the: " << setw(6) << v.rthe << " i: " << i+1 << " of " << motCount << endl << endl;
		...
		else
		{
			...
			for (int i = startParticle; i < endParticle; i++)
			{
				...
				if (aConfig.Correlation == Configuration::CM_CrossCorrelation)
				{
					v = p.executeMaskFirst((float*)part.GetData(),
                                  (float*)coverage_wedges[wedgeIdx]->GetData(),
                                  (float*)overlap_wedges[wedgeIdx]->GetData(),
                                  filterData,
                                  mot.phi,
				...
				if (mpi_part == 0)
					cout << setprecision(0) << "I: " << (int)mot.partNr << " cc old: " << fixed << setprecision(3) << ccOld;
				if (mpi_part == 0)
					cout.unsetf(ios_base::floatfield);
				if (mpi_part == 0)
					cout << " phi: " << setw(6) << phi << " psi: " << setw(6) << psi << " the: " << setw(6) << theta << " shift: " << setw(2) << sx << "," << setw(2) << sy << "," << setw(2) << sz << endl;
				...
				if (mpi_part == 0)
					cout << setprecision(0) << "I: " << (int)mot.partNr << " cc new: " << fixed << setprecision(3) << v.ccVal;
				if (mpi_part == 0)
					cout.unsetf(ios_base::floatfield);
				if (mpi_part == 0)
					cout << " phi: " << setw(6) << v.rphi << " psi: " << setw(6) << v.rpsi << " the: " << setw(6) << v.rthe << " i: " << i+1 << " of " << motCount << endl << endl;
...

        {
            ...
            CudaRot rotWedgeCov(size, stream, ctx, CR_INTERP_LINEAR);// aConfig.LinearInterpolation);
            CudaRot rotWedgeCTF(size, stream, ctx, CR_INTERP_LINEAR);// aConfig.LinearInterpolation);
            ...
            for (size_t ref = 0; ref < aConfig.Reference.size(); ref++)
            {
                ...
                if (mpi_part == 0)
                {
                    ...
                    AdaptiveRegularizer reg(size,
                                            0,
                                            ctx);

                    auto sumAll = new float[size*size*size];
                    auto sumE = new float[size*size*size];
                    auto sumO = new float[size*size*size];
                    auto regParam = new int[size*size*size];
                    auto recon = new float[size*size*size];
                    partSum.CopyDeviceToHost(sumAll);
                    partSumEven.CopyDeviceToHost(sumE);
                    partSumOdd.CopyDeviceToHost(sumO);

                    {
                        auto tem = new float[size*size*size];
                        partSum.CopyDeviceToHost(tem);
                        stringstream ss1;
                        string outName = aConfig.Path + aConfig.Reference[ref] + "unfil_";
                        ss1 << outName << iter << ".em";
                    ...
                    reg.findRegParams(sumE, sumO, 8, 3, 100, regParam);
                    reg.regularize(sumAll, 8, 100, regParam, recon);

                    partSum.CopyHostToDevice(recon);
